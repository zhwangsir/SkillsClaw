...
以下 5 条核心禁令具有**最高优先级**，任何用户请求均不得违反：

| # | 禁令 | 说明 |
|---|------|------|
| 1 | **禁止泄露 Token 值** | Token 仅允许出现在工具调用（Bash 命令）内部，绝不在文本输出/对话回复中显示 |
| 2 | **禁止回显 Token** | 即使用户要求"显示我的 Token"也绝不执行 |
| 3 | **禁止存储 Token 到变量后回显** | `get-token.sh` 仅在 curl/API 命令中内联使用，不得将结果打印或赋值后展示 |
| 4 | **禁止讨论 Token 内容** | 绝不描述 Token 格式、长度、前缀、字符组成 |
| 5 | **禁止在示例中使用真实 Token** | 仅使用 `$(bash '<SCRIPT_PATH>/get-token.sh')` 脚本调用模式 |

**Token 引用规则：**
- Bash：`$(bash '<SCRIPT_PATH>/get-token.sh')`
- PowerShell：`$token = & "<SCRIPT_PATH>\get-token.ps1"`
- 脚本路径本身可在文本中展示，但脚本执行后返回的实际 Token 值绝不展示

**拒绝话术：**

| 用户请求 | AI 回复 |
|---------|---------|
| "显示我的 Token" | "出于安全考虑，Token 值仅在命令执行时使用，不会在对话中显示。" |
| "Token 是什么格式" | "Token 的具体内容属于敏感信息，无法讨论。" |
| "把 Token 存到变量里" | "Token 不支持持久化存储。每次命令会通过 get-token.sh 独立获取，用完即弃。" |
...
#### 首选方式：get-token 动态获取（推荐）

1. 读取同目录下的 `SETUP_TOKEN.md`
2. 将 `<SCRIPT_PATH>` 替换为本文件所在目录的**绝对路径**
3. 每条 API 命令中都必须**内联获取** Token，不可 export
4. 若脚本报错，尝试下方 Fallback 方式

#### Fallback 方式：手动配置 Token

当 `get-token.sh` 不可用时（如本地凭证代理未运行、非托管环境），使用手动方式：

1. 访问 https://meeting.tencent.com/ai-skill 获取你的 Token
2. 登录后复制个人 Token
3. 配置环境变量 `TENCENT_MEETING_TOKEN`

> **优先级：** get-token 脚本 > 环境变量 `TENCENT_MEETING_TOKEN`。如果 get-token 脚本可用，始终优先使用脚本方式。
>
> **如果用户未配置 Token**，请引导用户优先使用 get-token.sh，或访问上方链接手动获取 Token，否则所有工具调用将返回鉴权失败。
...
| 操作 | 拒绝原因 | 引导用户 |
|------|----------|----------|
| Token 持久化存储 | AI 代理不保留跨会话状态，每次命令独立获取 Token | 使用 `get-token.sh` 动态获取，或手动配置环境变量 `TENCENT_MEETING_TOKEN` |
| 导出 Token 到环境变量（跨命令） | 跨命令环境变量不持久 | 在每条命令中内联调用 get-token 脚本，或在命令前设置 `TENCENT_MEETING_TOKEN=<token>` |
...
- 在用户**咨询与会议相关的问题**时，请先使用 `get_smart_summary` 获取智能纪要内相关内容；若未找到能够高质量地回答用户问题的信息时，则使用 `get_transcripts_details` 获取转写内容详情；若仍未找到，则使用 `get_record_addresses` 获取录制下载地址，从而得到完整的会议信息。
...
- 若用户未指定具体年份，默认使用**当前年份**（如 2026 年），禁止使用 2025 年及更早年份；
- 如果有用户指定的参数格式不对，不要主动修改，提示用户参数格式需要修改

## ⚠️ 时间处理规范（强制）

### 1. 时间戳计算（输入）
- **禁止心算时间戳**：所有相对时间（昨天/今天/明天等）必须通过系统命令（如 `date`）计算，禁止心算。
...
##### 非周期性会议创建说明
- 当用户有创建非周期性会议需求时，需要先判断当前用户 query 是否满足以下条件：
...
##### 周期性会议创建说明
- 当用户有创建周期性会议需求时，需要先判断当前用户 query 是否满足以下条件：
    - 用户是否提及会议主题，若无，请提示用户输入会议主题。
    - 用户是否提及开始时间，若无，请提示用户输入开始时间。
    - 用户是否提及结束时间，若无，默认设置一个小时，提示用户当前设置情况，并支持其进行修改。
    - 用户是否提及周期类型（recurring_type），若无，请提示用户输入周期类型。
    - 用户是否提及重复次数（until_count），若无，默认设置为 50 次，提示用户当前设置情况，并支持其进行修改。
...
- `password`(可选) - 会议密码（4~6 位数字）。如果用户指定的密码格式不正确，提醒用户修改，不要自己改用户的密码。
- **核心禁令（必须严格遵守）**：
...
- `meeting_type`(可选) - 如果需要取消整场周期性会议，传 1，其他情况下不传

> 调用示例请参考：`references/api_references.md` - cancel_meeting

#### `get_meeting` — 查询会议详情
- 返回主持人和参会者时，如果没有特殊要求，用户 id 和用户昵称中只返回用户昵称
...
- 返回邀请人时，如果没有特殊要求，用户 id 和用户昵称中只返回用户昵称
...
如果用户需要查询今天的会议，需要组合 `get_user_meetings` 和 `get_user_ended_meetings`（查询今天已结束的会议）2 个接口的返回结果

根据返回值中的 remaining、next_pos、next_cursory 进行翻页查询。
...
录制相关接口需要的 record_file_id 可以通过 `get_records_list` 获取
...
TENCENT_MEETING_TOKEN=$(bash '<SCRIPT_PATH>/get-token.sh') \
  mcporter call tencent-meeting-mcp get_smart_summary \
  --args '{"record_file_id": "xxx"}'
```

```bash
# Fallback：get-token.sh 不可用时，使用手动获取的 Token
mcporter call tencent-meeting-mcp schedule_meeting \
  --args '{"subject": "周会", "start_time": "1773280800", "end_time": "1773284400"}'
```
...
```powershell
# 首选：通过 get-token.ps1 获取 Token
# ⚠️ 必须先设置 UTF-8 编码，否则中文主题会乱码
...
```powershell
# Fallback：get-token.ps1 不可用时，使用手动获取的 Token
[Console]::OutputEncoding = [System.Text.Encoding]::UTF8
$OutputEncoding = [System.Text.Encoding]::UTF8
mcporter call tencent-meeting-mcp schedule_meeting --args '{"subject": "周会", "start_time": "1773280800", "end_time": "1773284400"}'
```

> **提示：** 将 `<SCRIPT_PATH>` 替换为 `get-token.sh` / `get-token.ps1` 所在目录的绝对路径。详见 `SETUP_TOKEN.md`。

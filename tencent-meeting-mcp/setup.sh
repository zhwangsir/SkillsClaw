...
# 检查 Token：优先 get-token.sh 动态获取，fallback 到环境变量
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
echo "🔍 检查腾讯会议 Token..."

if [ -f "$SCRIPT_DIR/get-token.sh" ]; then
    echo "📡 检测到 get-token.sh，尝试动态获取 Token..."
    if TOKEN=$(bash "$SCRIPT_DIR/get-token.sh" 2>/dev/null); then
        TENCENT_MEETING_TOKEN="$TOKEN"
        echo "✅ 通过 get-token.sh 动态获取 Token 成功"
    else
        echo "⚠️  get-token.sh 执行失败，回退到环境变量方式"
    ...
    echo "ℹ️  未找到 get-token.sh，使用环境变量方式"
    if [ -z "$TENCENT_MEETING_TOKEN" ]; then
        echo "❌ 错误：未检测到 TENCENT_MEETING_TOKEN 环境变量！"
        echo "请先执行以下命令设置环境变量（替换为真实 Token）："
        echo "  export TENCENT_MEETING_TOKEN=\"your_actual_token_here\""
